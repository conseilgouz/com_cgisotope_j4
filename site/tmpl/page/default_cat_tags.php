	    ...
	    if  (($displayfiltercat == "button")  || ($displayfiltercat == "multi") || ($displayfiltercat == "multiex")) {
		    ...
		    foreach ($sortFilter as $key => $filter) {
				...
				if (!is_null($aff)) {
					...
					$filter_cat_div .= '<button class="'.$button_bootstrap.'  iso_button_cat_'.$aff_alias.' '.$checked.'" data-sort-value="'.$aff_alias.'" title="'.$this->cats_note[$key].'"/>'.$img.Text::_($aff).'</button>'; // 2.3.2
				}
			}
			$filter_cat_div .= '</div>';
		} else {
	    ...
	    if (($displayfiltertags == "button") || ($displayfiltertags == "multi") || ($displayfiltertags == "multiex") ) {
			...
			foreach ($sortFilter as $aval) {
				...
				if (!is_null($aff)) {
					...
					$filter_tag_div .= '<button class="'.$button_bootstrap.'  iso_button_tags_'.$aff_alias.' '.$checked.'" data-sort-value="'.$aff_alias.'" title="'.$this->tags_note[$aff_alias].'"/>'.$img.Text::_($aff).'</button>'; // 2.3.2
				}
			}
...
foreach ($this->list as $key=>$category) {
	foreach ($category as $item) {
		...
		if ($this->iso_entree == "webLinks") {
			...
			$arr_css= array("{title}"=>$title, "{cat}"=>$this->cats_lib[$item->catid],"{date}"=>$libcreated.JHtml::_('date', $item->created, $libdateformat), "{visit}" =>$item->hits, "{intro}" => $item->description,"{tagsimg}" => $tag_img,"{catsimg}" => $cat_img, "{link}" => $item->link, "{introimg}"=>$item->introimg, "{subtitle}" => $item->subtitle, "{new}" => $item->new, "{tags}" => $itemtags,"{featured}" => $item->featured); 
