            ...
            if (($displayfiltertags == "button") || ($displayfiltertags == "multi") || ($displayfiltertags == "multiex")) {
                ...
                foreach ($sortFilter as $aval) {
                    ...
                    if (!is_null($aff)) {
                        ...
                        $tagtitle = "";
                        if (array_key_exists($aff_alias, $this->tags_note) && $this->tags_note[$aff_alias]) {
                            $tagtitle = 'title="'.$this->tags_note[$aff_alias].'"';
                        }
                        $tagcount = '';
                        if ($tagsfiltercount == 'true') {
                            $tagcount = '<span class="tag-count badge bg-info">'.$this->tags_count[$aff_alias].'</span>';
                        }
                        $filter_tag_div .= '<button class="'.$button_bootstrap.'  iso_button_tags_'.$aff_alias.' '.$checked.'" data-sort-value="'.$aff_alias.'" '.$tagtitle.'>'.$img.Text::_($aff).$tagcount.'</button>';
                    }
                }
                if ($tagsfilterparent == "true") {
                    $filter_tag_div .= '</div>';
...
foreach ($this->list as $key => $category) {
    foreach ($category as $item) {
        ...
        if (($article_cat_tag  == "tags") || ($article_cat_tag  == "cattags")) { // filtre tag
            ...
            foreach ($this->article_tags[$item->id] as $tag) {
                ...
                if ($this->tags_list && (count($this->tags_list) > 0) && ($this->params->get('tagsmissinghidden', 'false') == 'true')) {
                    $isdefined = CGHelper::checkTagSet($tag->tag, $filters['tags']) || $isdefined;
                }
        ...
        $itemtags = "";
        foreach ($this->article_tags[$item->id] as $tag) {
            $itemtags .= '<span class="iso_tag_'.$this->tags_alias[$tag->tag].'"><a href="'.$this->tags_link[$tag->alias].'">'.(($itemtags == "") ? $tag->tag : "<span class='iso_tagsep'><span>-</span></span>".$tag->tag).'</a></span>';
        }
